...
<nav class="lg:hidden fixed bottom-0 left-0 right-0 w-full bg-white/95 backdrop-blur-md border-t border-surface-variant/20 flex items-center justify-around px-2 py-1.5 z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
    ...
    <a href="{{ route('student.dashboard') }}" 
       class="flex flex-col items-center justify-center flex-1 py-1 transition-colors duration-150 {{ request()->routeIs('student.dashboard') ? 'text-primary' : 'text-slate-400 hover:text-slate-600' }}">
    ...
    <a href="{{ route('student.logs.index') }}" 
       class="flex flex-col items-center justify-center flex-1 py-1 transition-colors duration-150 {{ request()->routeIs('student.logs.*') ? 'text-primary' : 'text-slate-400 hover:text-slate-600' }}">
    ...
    <a href="{{ route('student.requirements') }}" 
       class="flex flex-col items-center justify-center flex-1 py-1 transition-colors duration-150 {{ request()->routeIs('student.requirements*') ? 'text-primary' : 'text-slate-400 hover:text-slate-600' }}">
    ...
    <a href="{{ route('student.profile') }}" 
       class="flex flex-col items-center justify-center flex-1 py-1 transition-colors duration-150 {{ request()->routeIs('student.profile*') ? 'text-primary' : 'text-slate-400 hover:text-slate-600' }}">
